...
def remodelling(
    ...
    for thr, key in zip(thresholds, keyMask):
        ...
        series.data['_seg'] = []
        series.data['_seg'] = [
            im > thr for im in deepcopy(series.data['_filt_'+keyImage])]
        
        # limit the segmented image to the masked region before transformation
        series.data['_seg'] = [im.mask(m.data) for im, m in zip(deepcopy(series.data['_seg']), deepcopy(series.data[key]))]
        ...
        common_region = binary_erosion(
            np.all([np.any(im.domain, axis=0) for im in deepcopy(segImages)], axis=0))
        ...
        boneVol = []
        totalVol = []
        for bv in deepcopy(segData):
            boneVol.append(np.sum(bv > 0.5))
